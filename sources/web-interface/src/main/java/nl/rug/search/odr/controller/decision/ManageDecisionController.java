...
public class ManageDecisionController extends AbstractController {
    ...
    private void setUpDecisionSpecific(RequestAnalyserDto requestAnalyser) {
        ...
        for (Decision currentDecision : project.getDecisions()) {
            if (currentDecision.getId().equals(decisionId)) {
                dl.makeTransient(currentDecision);
                this.decision = currentDecision;
                dl.makeTransient(this.decision);
                this.initialDecisionName = currentDecision.getName();
                break;
            }
        }
        ...
        for (Version currentVersion : decision.getVersions()) {
            if (currentVersion.getId().equals(versionId)) {
                vl.makeTransient(currentVersion);
                this.version = currentVersion;

                vl.makeTransient(this.version);
                initialState = currentVersion.getState();
                if (getStepId(currentStep) != 0) {
                    setStep(0);
                    JsfUtil.addJavascriptCall("odr.refresh();");
                }
