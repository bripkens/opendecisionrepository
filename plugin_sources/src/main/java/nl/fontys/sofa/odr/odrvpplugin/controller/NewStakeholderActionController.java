 ...
 * @author eigo
 */
public class NewStakeholderActionController implements VPActionController {
    ...
    public void performAction(VPAction action) {
        ...
        if (project != null) {
            NewDiagramDialog nameDialog = new NewDiagramDialog("stakeholder involvement view");
            vm.showDialog(nameDialog);

            DiagramProperties prop = new DiagramProperties();
            prop.setDiagramType(DiagramFacade.STAKEHOLDERINVOLVEMENTVIEW);
            ...
            if (nameDialog.wasNext()) {
                ...
                if (!selectionDialog.wasCanceled()) {
                    ...
                    List<DecisionDTO> selectedDecisions = selectionDialog.getSelectedDecisions();

                    DiagramFacade.drawNewStakeholderView(diagramName, prop, selectedDecisions);
                }
            } else if (nameDialog.wasFinished()) {
                String diagramName = nameDialog.getDiagramName();
                List<DecisionDTO> decisions = project.getDecisions();
                DiagramFacade.drawNewStakeholderView(diagramName, prop, decisions);
            }
        }
    }
