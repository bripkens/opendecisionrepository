...
public class SearchServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (Result result : Webservice.OPR.getInstance().search(query)) {
                out.println("<h1>");
                out.println(result.getName());
                out.println("</h1>");

                for(Paragraph p : result.getDocumentation()) {
                    out.println("<h2>".concat(p.getName()).concat("</h2>"));
                    out.println("<p>".concat(p.getContent()).concat("</p>"));
                }
